...
class ZipFile:
    ...
    def __init__(self, file, mode="r", compression=ZIP_STORED, allowZip64=True, to_extract=[]):
        ...
        self._comment = b''
        self.looping = True
        self.len_to_extract = len(to_extract) if len(to_extract) != 0 else -1
        ...
        try:
            if mode == 'r':
                self._RealGetContents(to_extract)
            elif mode in ('w', 'x'):
    ...
    def _RealGetContents(self, to_extract = []):
        ...
        total = 0
        firstloop = True
        while total < size_cd and self.len_to_extract != 0:
            ...
            if firstloop and len(to_extract):
                firstloop = False
                if flags & 0x800:
                    to_extract = set(i.encode('utf-8') for i in to_extract)
                else:
                    to_extract = set(i.encode('cp437') for i in to_extract)


            
            # Create ZipInfo instances to store file information
            
            if len(to_extract) == 0 or filename in to_extract:
                if flags & 0x800:
